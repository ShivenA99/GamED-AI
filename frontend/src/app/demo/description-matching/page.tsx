...
import { descriptionMatchingDemo } from '@/components/templates/InteractiveDiagramGame/data';
...
export default function DescriptionMatchingDemoPage() {
  const { resolvedTheme } = useTheme();
  const isDark = (resolvedTheme || 'dark') === 'dark';

  const handleComplete = useCallback((score: number) => {
    console.log('[DescriptionMatching Demo] Final score:', score);
  }, []);
    ...
    <div className={`min-h-screen py-8 px-4 ${isDark ? 'bg-background' : 'bg-background'}`}>
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-6 text-center">
          ...
          <p className="text-muted-foreground text-sm">Match each part of a flower to its description</p>
        </div>
        <InteractiveDiagramGame
          blueprint={descriptionMatchingDemo}
