...
import { tracePathDemo } from '@/components/templates/InteractiveDiagramGame/data';
...
export default function TracePathDemoPage() {
  const { resolvedTheme } = useTheme();
  const isDark = (resolvedTheme || 'dark') === 'dark';

  const handleComplete = useCallback((score: number) => {
    console.log('[TracePath Demo] Final score:', score);
  }, []);
    ...
    <div className={`min-h-screen py-8 px-4 ${isDark ? 'bg-background' : 'bg-background'}`}>
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-6 text-center">
          ...
          <p className="text-muted-foreground text-sm">Trace the path of food through the digestive system</p>
        </div>
        <InteractiveDiagramGame
          blueprint={tracePathDemo}
