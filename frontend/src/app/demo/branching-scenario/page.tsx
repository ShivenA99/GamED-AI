...
import { branchingDemo } from '@/components/templates/InteractiveDiagramGame/data';
...
export default function BranchingScenarioDemoPage() {
  const { resolvedTheme } = useTheme();
  const isDark = (resolvedTheme || 'dark') === 'dark';

  const handleComplete = useCallback((score: number) => {
    console.log('[BranchingScenario Demo] Final score:', score);
  }, []);
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-6 text-center">
          ...
          <h1 className={`text-2xl font-bold mb-1 ${isDark ? 'text-foreground' : 'text-foreground'}`}>
            Branching Scenario Demo
          </h1>
          <p className="text-muted-foreground text-sm">Make diagnostic decisions for a patient with chest pain</p>
        </div>
        <InteractiveDiagramGame
          blueprint={branchingDemo}
