...
import { clickToIdentifyDemo } from '@/components/templates/InteractiveDiagramGame/data';
...
export default function ClickToIdentifyDemoPage() {
  const { resolvedTheme } = useTheme();
  const isDark = (resolvedTheme || 'dark') === 'dark';

  const handleComplete = useCallback((score: number) => {
    console.log('[ClickToIdentify Demo] Final score:', score);
  }, []);
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-6 text-center">
          ...
          <h1 className={`text-2xl font-bold mb-1 ${isDark ? 'text-foreground' : 'text-foreground'}`}>
            Click to Identify Demo
          </h1>
          <p className="text-muted-foreground text-sm">Identify each part of a flower when given its function</p>
        </div>
        <InteractiveDiagramGame
          blueprint={clickToIdentifyDemo}
